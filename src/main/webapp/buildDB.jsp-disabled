...
<sql:update var="dropScopeResets" dataSource="jdbc/codeigniter">
    DROP TABLE IF EXISTS scoperesets
</sql:update>

<sql:update var="dropRifles" dataSource="jdbc/codeigniter">
    DROP TABLE IF EXISTS rifles
</sql:update>

<sql:update var="createScopeResetsTable" dataSource="jdbc/codeigniter">
    CREATE TABLE `scoperesets` (
    `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT,
    `rifleid` int(15) NOT NULL,
    `loadid` int(15) NOT NULL,
    `clicksfrombottomelev` int(15) NOT NULL,
    `clicksfrombottomwind` int(15) NOT NULL,
    `lastconfirmed` timestamp NOT NULL DEFAULT current_timestamp(),
    PRIMARY KEY  (`id`)
    )
</sql:update>

<sql:update var="createRiflesTable" dataSource="jdbc/codeigniter">
    CREATE TABLE `rifles` (
    `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT,
    `rifleowner` varchar(250) NOT NULL,
...
<sql:update var="dropScopeResetInfoView" dataSource="jdbc/codeigniter">
    DROP view IF EXISTS ScopeResetInfo
</sql:update>

<sql:update var="createScopeResetInfoView" dataSource="jdbc/codeigniter">
    create view ScopeResetInfo as 
    select sr.lastconfirmed, sr.clicksfrombottomelev as "ElevationClicks", sr.clicksfrombottomwind as "WindageClicks",  
    aml.BulletWeightGR, aml.avgSpeed, aml.name,
    rifles.rifleowner, rifles.description
    from scoperesets as sr
    inner join AllMyLoads aml
    on sr.loadid = aml.id
    inner join rifles
    on rifles.id = sr.rifleid
</sql:update>

<sql:update var="insertResets" dataSource="jdbc/codeigniter">
    INSERT INTO `rifles` (`id`, `rifleowner`, `description`) VALUES
    (1, 'sven', '308 win RugerAmerican (Nikon Buckmaster II, bdc)'),
    (2, 'sven', '308 win Springfield Armory M1A, Tactical Elite)');
</sql:update>
...
<sql:query var="insertAllMyLoads" dataSource="jdbc/codeigniter">
    INSERT INTO `AllMyLoads` (`GroupSize`,`barrelLenInches`,`name`,`ProjectileName`,`BulletWeightGR`,`diameter`,`Powder`,`GrainsUsed`,`EnergyFootLBS`,`maxSpeed`,`minSpeed`,`avgSpeed`,`StandardDeviation`,`loadCount`,`cycleFailureCount`,`failureToFeedCount`,`coal`,`avgGroup`) VALUES 
    (-1,-1,'.223','Varmint Grenade',50,'.224','IMR4064',23,940,2943,2889,2910,14,30,0,0,-1,-1),
    (-1,-1,'.223','SP Bulk',55,'.224','IMR4198',20,927,2807,2687,2754,35,20,0,0,-1,-1),
    (0.72,-1,'.223','FMJBT Bulk',55,'.224','IMR4064',24,873,2724,2634,2674,25,10,0,0,2.23,0.72),
    (1.3,-1,'30-06','FMJ Bulk',150,'.308','IMR4064',49,2678,2835,2835,2835,15,50,0,0,-1,1.3),
    (-1,-1,'30-06','A-Max',178,'.308','IMR4064',45,2329,2461,2390,2427,28,20,0,0,-1,-1),
    (-1,-1,'300 BLK','FMJ Bulk',150,'.308','A1680',21,1333,2016,1985,2000,10,10,0,0,-1,-1),
    (0.58,-1,'300 BLK','TAC-TX',110,'.308','Lil Gun',19,1326,2330,2998,2330,23,20,0,0,2.19,0.58),
    (-1,-1,'300 BLK','Rem OTFB',120,'.308','FACTORY',0,1178,2127,2082,2102,-1,40,0,0,-1,-1),
    (-1,-1,'300 BLK','Match Monster',155,'.308','Lil Gun',14,1166,1840,1840,1840,1,100,0,0,-1,-1),
    (-1,-1,'300 BLK','FMJ Bulk',150,'.308','IMR4227',15,936,1697,1655,1676,9,20,0,7,-1,-1),
    (-1,-1,'308 WIN','SST',165,'.308','IMR4064',40,2379,2569,2474,2525,30,15,0,0,-1,-1),
    (0.64,-1,'308 WIN','SST',150,'.308','IMR4064',42,2261,2645,2576,2605,27,20,0,0,-1,0.64),
    (-1,-1,'308 WIN','FMJ Bulk',150,'.308','IMR4895',38,1943,2452,2391,2415,21,15,0,0,-1,-1),
    (-1,-1,'308 WIN','FMJ Bulk',150,'.308','Trail Boss',13,614,1391,1327,1362,31,6,6,6,-1,-1),
    (-1,-1,'35 REM','Bulk FMJ PLD',158,'.357','Unique',10,728,1496,1404,1440,-1,10,0,0,-1,-1),
    (-1,-1,'35 REM','Bulk FMJ PLD',158,'.357','Trail Boss',11,657,1433,1151,1368,-1,10,0,0,-1,-1),
    (-1,-1,'357 Mag','Bulk FMJ PLD',158,'.357','IMR4227',14,841,1548,1548,1548,1,20,0,0,-1,-1),
    (-1,-1,'357 Mag','Bulk FMJ PLD',158,'.357','Unique',6,375,1034,1034,1034,1,20,0,0,-1,-1),
    (-1,-1,'38 Spc','Bulk FMJ PLD',158,'.357','Unique',5,233,815,815,815,1,20,0,0,-1,-1),
    (-1,-1,'40 S&W','Bulk FMJ FL',180,'.400','Unique',5,346,930,930,930,1,800,0,0,-1,-1),
    (-1,-1,'45 ACP','Bulk FMJ RN',220,'.45','Unique',5,285,771,754,764,-1,5,0,0,-1,-1)
</sql:query>
